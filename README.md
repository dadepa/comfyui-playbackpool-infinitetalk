...
InfiniteTalk renders can take longer than RunPod's short defaults. Configure both
timeouts on the Serverless endpoint:

```env
COMFY_TIMEOUT_SECONDS=3600
```

Also set the RunPod endpoint execution timeout to `3600` seconds. For production-length
training videos, split audio into short segments and merge the resulting clips instead of
rendering the full audio as one ComfyUI prompt.
